...
struct singleFrameResult {
  ...
  singleFrameResult(const Eigen::MatrixXd& patternPoints, const Eigen::Matrix2Xd& detectPoints)
...
void run(const InputArgs& args) {
  ...
  std::vector<std::shared_ptr<const IsometricGridDot>> pat = matcher->GetPatterns();
  singleFrameResult detectedResult(
      pat.at(0).get()->GetPattern(), res.detections[0].correspondences);
  ...
  if (!args.outputPath.empty()) {
    ...
    for (int i = 0; i < detectedResult.pattern.cols(); ++i) {
      if (!detectedResult.detectCorrespondence.col(i).hasNaN()) {
        ...
        ofs << "coor in camera space: (" << detectedResult.detectCorrespondence(0, i) << ","
...
int main(int argc, char** argv) {
  ...
  app.add_option("-i,--input-image", args.inputFile, "Input image.")->required();
  app.add_option("--pattern-file", args.patternSVG, "Pattern svg file.")->required();
  app.add_option(
  ...
  app.add_option(
         "--output_dir",
         args.outputPath,
         "directory for outputting the pattern and corresponding detected points.")
      ->required();
