...
#pragma once
// to estimate T_camera_target to transfer the image points to target space that the distance
// between neighrous = 1 please refer to this doc for more details:
...
class isometricPatternPoseCost {
 public:
  isometricPatternPoseCost(
      const Ray3d rayInCamera,
  ...
  bool operator()(const Scalar* const paramTargetPose, Scalar* residuals) const {
    const Eigen::Map<const Sophus::SE3<Scalar>> T_camera_target(paramTargetPose);
    Ray3<Scalar> rayInTarget = T_camera_target.inverse() * rayInCamera_.template cast<Scalar>();
    Ray3<Scalar> rayNeighbourInTarget =
        T_camera_target.inverse() * rayNeighbourInCamera_.template cast<Scalar>();
    ...
    Eigen::Vector<Scalar, 3> ptNeighbourTarget3d =
        rayNeighbourInTarget.line().intersectionPoint(plane_.template cast<Scalar>());
...
class isometricPatternDistortionCost {
 public:
  isometricPatternDistortionCost(
      const Eigen::Vector2d& imageDots,
  ...
  template <typename Scalar>
  bool operator()(
      const Scalar* const paramTargetPose,
    ...
    intrinsics << Scalar(focalLength_), Scalar(focalLength_), Scalar(centerXY_(0)),
        Scalar(centerXY_(1)), paramDistortion[0], paramDistortion[1], paramDistortion[2],
        paramDistortion[3];

    Ray3<Scalar> rayInCamera = KannalaBrandtK3Projection::unproject(
        imageDots_.template cast<Scalar>(), intrinsics); //.template cast<Scalar>();
    Ray3<Scalar> rayNeighbourInCamera = KannalaBrandtK3Projection::unproject(
        neighbourDots_.template cast<Scalar>(), intrinsics); //.template cast<Scalar>();

    const Eigen::Map<const Sophus::SE3<Scalar>> T_camera_target(paramTargetPose);
    Ray3<Scalar> rayInTarget = T_camera_target.inverse() * rayInCamera.template cast<Scalar>();
    Ray3<Scalar> rayNeighbourInTarget =
        T_camera_target.inverse() * rayNeighbourInCamera.template cast<Scalar>();
