...
struct DefaultImageValTraits<T, typename std::enable_if<std::is_integral<T>::value>::type> {
  static constexpr int max_value = (sizeof(T) >= sizeof(int))
...
//
// MaxValue is a compile-time hint, which indicates the maximum value, if this structure represents
// an intensity image. This class is CUDA aware: it does not require CUDA, but supports being
// compiled in CUDA. If HAVE_CUDA is defined, then some operations will use CUDA functions.
...
struct Image {
  ...
  inline Image(T* ptr, size_t w) noexcept : pitch(sizeof(T) * w), ptr(ptr), w(w), h(1) {}
  inline Image(T* ptr, size_t w, size_t h) noexcept : pitch(sizeof(T) * w), ptr(ptr), w(w), h(h) {}
  ...
  inline Eigen::Vector2i Dim() const {
  ...
  template <
      typename TOther,
      int OtherMaxValue,
      typename std::enable_if<std::is_same<T, typename std::remove_cv<TOther>::type>::value, int>::
          type = 0>
  ...
  template <
      typename R,
      typename std::enable_if<std::is_integral<BaseType>::value, typename R::Base::BaseType>::type =
          0,
      typename std::enable_if<std::is_integral<typename R::Base::BaseType>::value, int>::type = 0>
  inline R ConvertTo() const {
    ...
    static_assert(((R::Base::max_value + 1) & R::Base::max_value) == 0, "Must be power of 2");
    ...
    // Sanity check
    static_assert((max_value + 1) % (R::Base::max_value + 1) == 0, "Should be divisble");
    ...
    static constexpr int CONVERSION_FACTOR = (max_value + 1) / (R::Base::max_value + 1);
  ...
  bool InImage(const T* ptest) const {
    return ptr <= ptest && ptest < end();
  }

  inline bool YInBounds(int y) const {
    return 0 <= y && y < (int)h;
  }
  ...
  inline bool InBounds(
    ...
    EIGEN_STATIC_ASSERT(
        Derived::RowsAtCompileTime == 2, THIS_METHOD_IS_ONLY_FOR_MATRICES_OF_A_SPECIFIC_SIZE);
  ...
  inline const Image<const T, MaxValue> SubImage(size_t x, size_t y, size_t width, size_t height)
      const {
    CHECK((x + width) <= w && (y + height) <= h);
    return Image<const T, MaxValue>(RowPtr(y) + x, width, height, pitch);
  }

  inline Image<T, MaxValue> SubImage(size_t x, size_t y, size_t width, size_t height) {
    CHECK((x + width) <= w && (y + height) <= h);
    return Image<T, MaxValue>(RowPtr(y) + x, width, height, pitch);
  }

  inline const Image<const T, MaxValue> SubImage(const Eigen::AlignedBox2i& b) const {
  ...
  inline Image<T, MaxValue> Row(int y) {
    return SubImage(0, y, w, 1);
  }
  inline Image<T, MaxValue> Col(int x) {
    return SubImage(x, 0, 1, h);
  }
  ...
  inline Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>> GetEigenMap() {
    // to enable operations on images using eigen: e.g. im_map = im_map + im_map*2;
    Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>> im_map(ptr, h, w);
    return im_map;
  }

  inline Eigen::Map<const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>> GetEigenMap() const {
    // to enable operations on images using eigen: e.g. im_map = im_map + im_map*2;
    Eigen::Map<const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>> im_map(ptr, h, w);
    return im_map;
  }

  inline Eigen::
      Map<Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>, 0, Eigen::Stride<Eigen::Dynamic, 1>>
      GetEigenMapPitched() {
    // to enable operations on images using eigen: e.g. im_map = im_map + im_map*2;
    // use Eigen::Stride to work on subimages as well.
    Eigen::
        Map<Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>, 0, Eigen::Stride<Eigen::Dynamic, 1>>
            im_map(ptr, h, w, Eigen::Stride<Eigen::Dynamic, 1>(pitch / sizeof(T), 1));
    return im_map;
  }
...
static_assert(Image<uint16_t, 1023>::max_value == 1023, "Compile time sanity check");
static_assert(Image<float, 0>::max_value == 0, "Compile time sanity check");
static_assert(Image<Eigen::Matrix<uint8_t, 3, 3>>::max_value == 255, "Compile time sanity check");
static_assert(Image<Eigen::Matrix<float, 1, 2>>::max_value == 1, "Compile time sanity check");
static_assert(
    Image<Eigen::Matrix<uint16_t, 1, 3>, 1023>::max_value == 1023,
    "Compile time sanity check");
static_assert(Image<Eigen::Vector3d, -1>::max_value == -1, "Compile time sanity check");
...
class ManagedImage : public Image<T, MaxValue> {
 public:
  ...
  inline ManagedImage(size_t w) : Base(Allocator().allocate(w), w, 1, w * sizeof(T)) {
    CHECK(w != 0);
  }

  // Precondition: Neither w nor h should be 0.
  inline ManagedImage(size_t w, size_t h) : Base(Allocator().allocate(w * h), w, h, w * sizeof(T)) {
    CHECK(w != 0 && h != 0);
  }

  // Precondition: Neither dim.width nor dim.height should be 0.
  inline ManagedImage(ImageDimensions dim) : ManagedImage(dim.width, dim.height) {}
  ...
  // Move constructor
  inline ManagedImage(ManagedImage&& img) noexcept : Base(img.ptr, img.w, img.h, img.pitch) {
    img.ptr = nullptr;
  }
