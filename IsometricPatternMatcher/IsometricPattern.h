...
class IsometricGridDot {
 public:
  ...
  std::array<Eigen::MatrixXi, 6> makeIsometricPatternGroup(Eigen::MatrixXi pattern0);

  void LoadFromSVG(const std::string& SVGFile);

  bool IsXmlParamsString(std::string s);
  bool ParseXmlParamsString(
      const std::string& s,
   ...
   *
   * @return The location of the calibu points as a [3 x (col * row)] matrix. All points are
   *         projected to the z=0 plane.
   */
  const Eigen::MatrixXd& GetPattern() const {
    return patternPts_;
  }
   ...
   *
   * @return Vector of binary codes (1 or 0), entries are aligned with the points returned in the
   *         GetPattern() function.
   */
  ...
  inline size_t NumberLayer() const {
    return numberLayer_;
  }

  inline size_t gridRows() const {
    return gridRowsCols_[0];
  }

  inline size_t gridCols() const {
    return gridRowsCols_[1];
  }

  inline double verticalSpacing() const {
    return verticalSpacing_;
  }

  inline double horizontalSpacing() const {
    return horizontalSpacing_;
  }

  inline double dotRadius() const {
    return dotRadius_;
  }

  inline size_t storageMapRows() const {
    return storageMapRows_;
  }
