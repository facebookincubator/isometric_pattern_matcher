...
Eigen::MatrixXi IsometricGridDot::makeIsometricPattern(uint32_t seed) {
  ...
  for (int r = 0; r < M.rows(); ++r) {
    ...
    for (int q = 0; q < M.cols(); ++q) {
      int col = q - numberLayer_ + (r - numberLayer_ - ((r - numberLayer_) & 1)) / 2;

      M(r, q) = (r + q < numberLayer_ ||
                 r + q > 3 * numberLayer_ || // upper and lower triangles of the matrix
                 abs(row) > gridRowsCols_[0] / 2 ||
                 abs(col) > gridRowsCols_[1] / 2) // outside the specified rows and cols of the grid
          ? 2
          : uintDist1(rng);
...
Eigen::MatrixXi IsometricGridDot::Rotate60Right(Eigen::MatrixXi& inputPatternGrid) const {
  ...
  for (int r = 0; r < inputPatternGrid.rows(); ++r) {
    for (int q = 0; q < inputPatternGrid.cols(); ++q) {
      if (r + q >= numLayer && r + q <= 3 * numLayer)
        outputPatternGrid(r, q) = inputPatternGrid(2 * numLayer - q, r + q - numLayer);
    } // upper and lower triangles of the matrix should be Null, incidated by 2
  }
...
void IsometricGridDot::Init() {
  ...
  for (int r = 0; r < patternGroup_[0].rows(); ++r) {
    ...
    for (int c = 0; c < patternGroup_[0].cols(); ++c) {
      ...
      double x = centerX + ((c - centerIndx) + (r - centerIndx) / 2.0) * horizontalSpacing_;
      patternPts_.col(r * storageMapRows_ + c) = Eigen::Vector3d(x, y, 0);
    }
  }
}

IsometricGridDot::IsometricGridDot(
...
void IsometricGridDot::LoadFromSVG(const std::string& gridFile) {
  ...
  CHECK(svgFile.is_open()) << fmt::format("Unable to open Target SVG file, '{}'", gridFile);
  ...
  while (!svgFile.eof()) {
    ...
    if (IsXmlParamsString(line) == false)
      continue;

    bool success = ParseXmlParamsString(
        line,
        binaryPattern,
        numberLayer_,
        gridRowsCols_,
    ...
    CHECK(success) << fmt::format("Unable to parse grid file {}", gridFile.c_str());
    break;
  }

  CHECK(!svgFile.eof()) << "Grid file {} does not contain target parameters.", gridFile.c_str();
...
bool IsometricGridDot::ParseXmlParamsString(
  ...
  if (!ParseOption(s, "-vertical-spacing ", verticalSpacing)) {
    LOG(ERROR) << "Xml comment in svg pattern file does not contain -vertical-spacing field.";
    return false;
  }

  if (!ParseOption(s, "-horizontal-spacing ", horizontalSpacing)) {
    LOG(ERROR) << "Xml comment in svg pattern file does not contain -horizontal-spacing field.";
    return false;
  }

  if (!ParseOption(s, "-layer-number ", numberLayer)) {
    LOG(ERROR) << "Xml comment in svg pattern file does not contain -layer-number field.";
    return false;
  }

  if (!ParseOption(s, "-rows ", gridRowsCols[0])) {
    LOG(ERROR) << "Xml comment in svg pattern file does not contain -rows field.";
    return false;
  }

  if (!ParseOption(s, "-cols ", gridRowsCols[1])) {
    LOG(ERROR) << "Xml comment in svg pattern file does not contain -cols field.";
    return false;
  }

  if (!ParseOption(s, "-grid-dotRadius ", dotRadius)) {
    LOG(ERROR) << "Xml comment in svg pattern file does not contain -grid-dotRadius field.";
    return false;
  }

  if (!ParseOption(s, "-grid-pattern ", binaryPattern)) {
    LOG(ERROR) << "Xml comment in svg pattern file does not contain -grid-pattern field.";
    return false;
  }

  if (std::ceil((numberLayer * 2 + 1) * (numberLayer * 2 + 1)) != binaryPattern.length()) {
    LOG(ERROR) << "Pattern string length does not agree with the specified grid size.";
    return false;
  }
...
template <typename T>
bool IsometricGridDot::ParseOption(const std::string& s, const std::string& key, T& val) {
...
void IsometricGridDot::SaveSVG(
  ...
  f << ISOMETRIC_PATTERN_TRAILING_STRING << " -vertical-spacing " << verticalSpacing_
    << "  -horizontal-spacing " << horizontalSpacing_ << "  -layer-number " << numberLayer_
    << "  -rows " << gridRowsCols_[0] << "  -cols " << gridRowsCols_[1] << "  -grid-dotRadius "
    << dotRadius_;
  f << "  -grid-pattern ";
  for (int r = 0; r < M.rows(); ++r) {
    for (int q = 0; q < M.cols(); ++q)
  ...
  f << " -->" << std::endl;
  // units in mm
  const double canvasWidth = ((gridRowsCols_[1] - 1) * horizontalSpacing_ * 1000) + offsetInMm * 2.;
  const double canvasHeight = ((gridRowsCols_[0] - 1) * verticalSpacing_ * 1000) + offsetInMm * 2.;

  f << fmt::format(R"#(<svg width="{}mm" height="{}mm">)#", canvasWidth, canvasHeight);
  f << std::endl;
  f << fmt::format(
