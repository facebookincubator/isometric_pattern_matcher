...
Eigen::Matrix2Xd PatternMatcherIsometric::DotDetection(const Image<uint8_t>& image) const {
  DotExtractor extractor;
  ManagedImage<DotTypeFloat> dots;
  int numDots;
  Eigen::AlignedBox2i roi(
      Eigen::Vector2i(10, 10), Eigen::Vector2i(image.Width() - 10, image.Height() - 10));
...
PatternMatcherIsometric::PatternMatcherIsometric(
    ...
    const IsometricOpts& opts)
    : opts_(opts) {
  CHECK_GT(patternFiles.size(), 0)
      << fmt::format("Matcher needs at least one target, {} specified", patternFiles.size());

  isometricGrids_.push_back(std::make_shared<IsometricGridDot>(patternFiles));
}

std::vector<std::shared_ptr<const IsometricGridDot>> PatternMatcherIsometric::GetPatterns() const {
...
PatternMatcherIsometric::Result PatternMatcherIsometric::Match(
  ...
  for (int i = 0; i < detectedDots.cols(); ++i) {
    double max = 0;
    for (int r = -opts_.intensityWindowSize / 2; r <= opts_.intensityWindowSize / 2; ++r) {
      for (int c = -opts_.intensityWindowSize / 2; c <= opts_.intensityWindowSize / 2; ++c) {
        const auto* r0 = imageU8.RowPtr((int)detectedDots(1, i) + r) + (int)detectedDots(0, i) + c;
        if (*r0 > max) {
          max = *r0;
        }
      }
  ...
  int rotationIndx;
  int numberMatch = grid.findOffset(
      isometricGrids_[0]->GetBinaryPatternGroup(), grid.detectPattern(), offset, rotationIndx);
  fmt::print(
      "{} points can be matched from total {} detected points \n",
      numberMatch,
      detectedDots.cols());
  ...
  fmt::print("rotationIndx: {}, offset: ({}, {}).\n", rotationIndx, offset.x(), offset.y());
  ...
  for (int r = 0; r < detectedIndxMap.rows(); ++r) {
    for (int c = 0; c < detectedIndxMap.cols(); ++c) {
      if (r + offset.x() >= 0 && r + offset.x() < storageMapRows && c + offset.y() >= 0 &&
          c + offset.y() < storageMapRows) {
